...
if (!IsNil("_class") and !Isnil("_zone")) then
{
		switch(_class) do
		{
				//Infantry
				case "Infantry":
					{ _dummy= [_group, _zone] call gaia_fnc_DoPatrolInf;};
				case "ReconInfantry":
					{ _dummy= [_group, _zone] call gaia_fnc_DoPatrolRecon;};
				case "MechanizedInfantry":
					{ _dummy= [_group, _zone] call gaia_fnc_DoPatrolMechInf;};
				case "MotorizedInfantry":
					{ _dummy= [_group, _zone] call gaia_fnc_DoPatrolMotorInf;};
				case "Car":
					{ _dummy= [_group, _zone] call gaia_fnc_DoPatrolCar;};
				case "MotorizedRecon":
					{ _dummy= [_group, _zone] call gaia_fnc_DoPatrolMotorRecon;};
				case "Ship":
					{ _dummy= [_group, _zone] call gaia_fnc_DoPatrolShip;};
				case "Submarine":
					{ _dummy= [_group, _zone] call gaia_fnc_DoPatrolShip;};
				case "Helicopter":
					{ _dummy= [_group, _zone] call gaia_fnc_DoPatrolHeli;};
