		...
		{
			{
			   ...
			   then
			   {
			   			switch(_TransportClass) do
					{
							...
							case "Helicopter":
								{ _dummy= [_group, _TransportGrp] call gaia_fnc_DoTransportHeli;};
							case "Ship":
								{ _dummy= [_group, _TransportGrp] call gaia_fnc_DoTransportShip;};
			...
			}forEach ([AllGroups,[],{(leader _group) distance (leader _x)}, "ASCEND",{alive (leader _x)}] call gaia_fnc_SortGroupsByCA);
		};
	};
