...
_statics = [(nearestObjects [_position,["StaticWeapon"], _radius]), {(_x iskindof "StaticWeapon") && (isnull(assignedGunner (_x))) && {(position _x) inArea _zone} && {(_x emptypositions "Gunner" > 0)}}] call BIS_fnc_conditionalSelect;
//_buildings = _position nearObjects ["building", _radius];
_buildings = [(_position nearObjects ["building", _radius]), {(position _x) inArea _zone}] call BIS_fnc_conditionalSelect;
_units = units _group;
_count = count _units;
