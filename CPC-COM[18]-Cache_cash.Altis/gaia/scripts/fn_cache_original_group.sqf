...
if (({alive _x} count units _group) > 0) then {
	{
		...
		{
			if ( ( assignedVehicle _x == _veh) and ( alive _x) )
			then {
		...
		_gv = _gv + [[(typeof _veh), position _veh, damage _veh, fuel _veh,  _crew, getdir _veh]];
	} foreach _vehicles;
	...
	if (((count (waypoints _group)) - currentWaypoint _group) > 0) then {
		{
			...
			if (( (waypointposition _x) distance [0,0,0]) > 0) then {
				_waypoint = _x call gaia_fnc_export_waypoint;
