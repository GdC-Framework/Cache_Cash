	...
	then
	{
		  ...
		  if  (
			...
			if (
						 ...
						 (behaviour(leader _x)=="COMBAT")
						 and
						 !((([_HQ_side,([_CA,leader _x] call BIS_fnc_nearestPosition)] call gaia_fnc_GetCAPoints) select 0 )in ["Helicopter", "Autonomous"])
					)
...

{
	...
	_CaPoints = ([_side, _SelectCA] call gaia_fnc_GetCAPoints) select 2;
	...
	//Now for each group make sure we check what he needs to do.
	{
		if (
			...
			and (count(_x getVariable  ["GAIA_zone_intend",[]])>1)
				...
				if (count(_x getVariable  ["GAIA_zone_intend",[]])>1) then
				{
					_CanDoAttack = (
						...
						(
							...
							and
							(_SelectCA inArea ((_x getVariable  ["GAIA_zone_intend",[]]) select 0))
						)
					);
					...
					{
						if ([(position leader _grp ), _x] call gaia_fnc_PosIsInMarker) exitwith {_CanDoAttack=true};
					}foreach _zns;
					*/
				};
				...
				_CaHoldsValidTargets			= !((([_HQ_side, _SelectCA] call gaia_fnc_GetCAPoints)select 0) in ["Helicopter", "Autonomous"]);
		 ...
		 //Next CA
		 sleep 0.1;
	}forEach 	([AllGroups,[],{_SelectCA distance (leader _x)}, "ASCEND",{alive (leader _x)}] call gaia_fnc_SortGroupsByCA);
		...
		_SpotPos = _SpotInfo select 1;
		{
			if (
				...
				_SpotIsWithinRange = (
					...
					or
					(_SpotPos inArea ((_x getVariable  ["GAIA_zone_intend",[]]) select 0))
				);
				...
				switch (true) do {
					...
					case (
						...
						and  !_GroupIsFleeing
						and  !_GroupBusy
						and  !_SpotIsBeingCleared
						and  _SpotIsOutsideCA
						and  _SpotIsWithinRange ): {
							//Clear the area, something fishy happened here recently
							_dummy=[_x, _SpotPos] call gaia_fnc_DoClear;
						};
				};
			};
		} forEach ([AllGroups,[],{_SpotPos distance (leader _x)}, "ASCEND",{alive (leader _x)}] call gaia_fnc_SortGroupsByCA);
	};
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
{
		_grp = _x;
		   	 ...
		   	 then
		   	 {
		   	 		...
		   	 		{
		   	 			_SelectCA = _x;
						([_side, _SelectCA] call gaia_fnc_GetCAPoints) select 2;
							...
							and !((([_HQ_side, _SelectCA] call gaia_fnc_GetCAPoints)select 0) in ["Car", "Tank", "Helicopter", "Ship", "Support", "Autonomous", "Submarine"])
							//Dont fire on something we already fire on
							and !(({(side _x == (_HQ_side)) and alive (leader _x) and (( _x getVariable ["GAIA_MortarTarget",[0,0,0]] distance _SelectCA )==0)} count (allgroups-[_grp]))>0)
							and (
								(((_grp getVariable  ["GAIA_zone_intend",[]]) select 1)=="MOVE")
								or (
									(((_grp getVariable  ["GAIA_zone_intend",[]]) select 1)=="NOFOLLOW")
									and (_SelectCA inArea ((_grp getVariable  ["GAIA_zone_intend",[]]) select 0))
								)
							)
						) then {
							...
							if ((_grp getVariable ["GAIA_MortarRound",0])>0) then {
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
{
	if (count(_x getVariable  ["GAIA_zone_intend",[]])>1) then {
		if (
			...
			and (count(_x getVariable  ["GAIA_Portfolio",[]])>0)
			and (
				((_x getVariable  ["GAIA_Order", ""]) == "DoPatrol")
				and !((position leader _x) inArea ((_x getVariable  ["GAIA_zone_intend",["",[0,0,0]]]) select 0))
			)
			//You belong to a footmobile class or you wont be able to getin
			and ((_x getVariable  ["GAIA_class", ""]) in ["Infantry", "ReconInfantry"])
		...
		//If that is all the case, then go check if somebody can transport you and if you are allowed to be transported based on transport specific rule bases
			_dummy = [_x] call gaia_fnc_DoOrganizeTransportation };
	};
