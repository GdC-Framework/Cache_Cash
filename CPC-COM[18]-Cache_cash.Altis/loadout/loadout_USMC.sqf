...
hard_setLoadout = 
{
	...
	switch (cc_loadoutcamo) do {
		case "desert": {
			...
			_helmet = if (CC_p_nvg == 1) then {"rhsusf_lwh_helmet_marpatd"} else {selectrandom ["rhsusf_lwh_helmet_marpatd","rhsusf_lwh_helmet_marpatd_ess"]};
			_helmet_tl = if (CC_p_nvg == 1) then {"rhsusf_lwh_helmet_marpatd"} else {"rhsusf_lwh_helmet_marpatd_headset"};
